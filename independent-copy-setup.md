...
## Перенос уже накопленных данных

Если нужна не пустая база, а полная копия текущих данных — тот, у кого есть доступ к исходному Supabase-проекту, должен один раз выгрузить 3 файла (это не может сделать сам получатель копии, у него нет доступа к исходным данным): **Table Editor** → по очереди `sb_uploads`, `sb_snapshots`, `sb_actions` → **Export → Export as CSV**.

Дальше в НОВОМ проекте, **между шагами 3 и 4** этой инструкции: **Table Editor** → та же таблица → **Insert → Import data from CSV** → выбрать присланный файл. Порядок важен — **сначала `sb_uploads`, затем `sb_snapshots` и `sb_actions`** (иначе внешний ключ `upload_id` не свяжется).
